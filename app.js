...
const firebaseConfig = {
    ...
    if (user) {
        ...
        // Pequeno atraso para o Firebase estabilizar os tokens
    ...
    window.carregarTarefas = async () => {
        ...
        try {
            ...
            if (tipoFiltroTempo === 'semana') {
                ...
                const qPessoal = query(collection(db, "tarefas"), where("uid", "==", window.usuarioLogado.uid), where("dataString", "==", dataFiltro));
                ...
                if (meusTimesIds.length > 0) {
                    ...
                    const qTime = query(collection(db, "tarefas"), where("timeId", "in", meusTimesIds), where("dataString", "==", dataFiltro));
    ...
    try {
        ...
        // 1. Busca os times que eu pertenço
        if (meuEmail) {
            const qTimes = query(collection(db, "times"), where("membrosEmails", "array-contains", meuEmail));
            const snapTimes = await getDocs(qTimes);
            snapTimes.forEach(d => meusTimesIds.push(d.id));
        ...
        // 3. Busca categorias de times
        if (meusTimesIds.length > 0) {
            const qCatTime = query(collection(db, "categorias"), where("timeId", "in", meusTimesIds));
            const snapCatTime = await getDocs(qCatTime);
            snapCatTime.forEach(d => { categoriasUnicas.set(d.id, d.data()); });
        }

        // 4. Montagem do HTML das abas
        let categoriasAtivasIniciais = JSON.parse(localStorage.getItem('categoriasAgendaAtivas')) || ["Geral"];
        const taGeralAtiva = categoriasAtivasIniciais.includes("Geral");
        
        let htmlTabs = `<div class="category-tab" data-nome="Geral" 
            style="background-color: ${taGeralAtiva ? "#334155" : "rgba(255,255,255,0.2)"}; 
            color: ${taGeralAtiva ? "white" : "#1e293b"};" 
            onclick="selecionarCat('Geral', '#334155')">Geral</div>`;
